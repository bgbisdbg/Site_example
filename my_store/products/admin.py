...
class ProductAdmin(admin.ModelAdmin):     # Класс для отображение модели продуктов в админке наследуемого от
    ...
    fields = ('image', 'name', 'description', ('price', 'quantity'), 'stripe_product_price_id', 'category') # Переменная отвечающая за отображение в админке в самом продукте по строкам (можно менять местами)
